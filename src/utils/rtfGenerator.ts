...
export const parseInlineStylesToHTML = (text: string, inlineRules: InlineRule[]): string => {
  let html = text
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;');
  ...
  sortedRules.forEach(rule => {
    ...
    try {
      ...
      html = html.replace(regex, (_, content) => {
        return `<span class="inline-style-${rule.styleName} font-semibold transition-all duration-150 px-1 rounded-md bg-slate-50 border-b-2" style="color: ${color}; border-color: ${color};" title="Стиль символа: ${rule.styleName}">${content}</span>`;
      });
    } catch (e) {
      console.error(e);
    }
